...
(function (factory) {
    ...
    FRAMES = '.file-prev-frame:visible';
    ...
    DEFAULT_PREVIEW = '<div class="file-prev-other">\n' +
    ...
    previewCache = {
        ...
        get: function (id, i, isDisabled) {
            ...
            if (!isEmpty(config) && !isEmpty(config.frameAttr)) {
                ...
                $tmp.find('.file-prev-initial').attr(config.frameAttr);
                out = $tmp.html();
                $tmp.remove();
            }
            return out;
    ...
    tMain1 = '{preview}\n' +
        ...
        '</div>';
    tMain2 = '{preview}\n<div class="kv-upload-progress hide"></div>\n{remove}\n{cancel}\n{upload}\n{browse}\n';
    tPreview = '<div class="file-prev {class}">\n' +
        '    {close}' +
        '    <div class="{dropClass}">\n' +
        '    <div class="file-prev-thumbnails">\n' +
        '    </div>\n' +
        '    <div class="clearfix"></div>' +
        '    <div class="file-prev-status text-center text-success"></div>\n' +
    ...
    tTagBef = '<div class="file-prev-frame{frameClass}" id="{previewId}" data-fileindex="{fileindex}"' +
        ' data-template="{template}"';
    tTagBef1 = tTagBef + '><div class="kv-file-content">\n';
    tTagBef2 = tTagBef + ' title="{caption}" ' + STYLE_SETTING + '><div class="kv-file-content">\n';
    tTagAft = '</div>{footer}\n</div>\n';
    tGeneric = '{content}\n';
    tHtml = '<div class="kv-preview-data file-prev-html" title="{caption}" ' + STYLE_SETTING + '>{data}</div>\n';
    tImage = '<img src="{data}" class="kv-preview-data file-prev-image" title="{caption}" alt="{caption}" ' +
        STYLE_SETTING + '>\n';
    tText = '<textarea class="kv-preview-data file-prev-text" title="{caption}" readonly ' + STYLE_SETTING +
        '>{data}</textarea>\n';
    ...
    tOther = '<div class="kv-preview-data file-prev-other-frame">\n' + DEFAULT_PREVIEW + '\n</div>\n';
    ...
    FileInput.prototype = {
        ...
        _init: function (options) {
            ...
            self.$previewContainer = getElement(options, 'elPreviewContainer', self.$container.find('.file-prev'));
            self.$preview = getElement(options, 'elPreviewImage', self.$container.find('.file-prev-thumbnails'));
            self.$previewStatus = getElement(options, 'elPreviewStatus', self.$container.find('.file-prev-status'));
        ...
        _initClickable: function () {
            ...
            handler($zone, 'click', function (e) {
                var $target = $(e.target);
                if (!$target.parents('.file-prev-thumbnails').length || $target.parents(
        ...
        _clearPreview: function () {
            var self = this, $thumbs = !self.showUploadedThumbs ? self.$preview.find(FRAMES) :
                self.$preview.find(FRAMES + ':not(.file-prev-success)');
            $thumbs.remove();
        ...
        _getThumbs: function (css) {
            css = css || '';
            return this.$preview.find(FRAMES + ':not(.file-prev-initial)' + css);
        },
                ...
                if ($thumb !== undefined) {
                    if (!allFiles) {
                        ...
                        $thumb.fadeOut('slow', function () {
                            var $newThumb = $div.find('.file-prev-frame'),
        ...
        _initSuccessThumbs: function () {
            ...
            self._getThumbs('.file-prev-success:visible').each(function () {
        ...
        _uploadSingle: function (i, files, allFiles) {
            ...
            if (self.showPreview) {
                $thumb = $('#' + previewId + ':not(.file-prev-initial)');
            ...
            fnBefore = function (jqXHR) {
                ...
                if (self.showPreview) {
                    if (!$thumb.hasClass('file-prev-success')) {
        ...
        _uploadBatch: function () {
            ...
            fnSuccess = function (data, textStatus, jqXHR) {
                ...
                var outData = self._getOutData(jqXHR, data), $thumbs = self._getThumbs(':not(.file-prev-error)'), key = 0,
        ...
        _initFileActions: function () {
            ...
            self.$preview.find('.kv-file-remove:visible').each(function () {
                ...
                handler($el, 'click', function () {
                    ...
                    hasError = $frame.hasClass('file-prev-error');
                    cleanMemory($frame);
                    $frame.fadeOut('slow', function () {
            ...
            self.$preview.find('.kv-file-upload:visible').each(function () {
                ...
                handler($el, 'click', function () {
                    ...
                    if (!$frame.hasClass('file-prev-error')) {
        ...
        _setThumbStatus: function ($thumb, status) {
            ...
            var icon = 'indicator' + status, msg = icon + 'Title',
                css = 'file-prev-' + status.toLowerCase(),
                $indicator = $thumb.find('.file-upload-indicator'),
                config = self.fileActionSettings;
            $thumb.removeClass('file-prev-success file-prev-error file-prev-loading');
            if (status === 'Error') {
        ...
        disable: function () {
            ...
            self.$container.find(".btn-file, .fileinput-remove, .fileinput-upload, .file-prev-frame button").attr(
                "disabled",
                true);
            self._initDragDrop();
            return self.$element;
        ...
        enable: function () {
            ...
            self.$container.find(
                ".btn-file, .fileinput-remove, .fileinput-upload, .file-prev-frame button").removeAttr("disabled");
            self._initDragDrop();
            return self.$element;
        },
        upload: function () {
